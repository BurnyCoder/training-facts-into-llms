...
\hypersetup{
  ...
  citecolor=LinkBlue,
  urlcolor=LinkBlue,
  pdftitle={Teaching One Synthetic Fact to Qwen3.8-27B: A Minimal LoRA Case Study with BF16 Compute},
...
\title{Teaching One Synthetic Fact to Qwen3.8-27B: A Minimal LoRA Case Study with BF16 Compute}
\author{Libor Burian}
...
specificity and a prespecified set of controls. The untouched base scored
...
\PaperScores{11/12}{8/8}{8/8}. It passed the source-declared acceptance policy,
which was committed before the invocation \citep{qwen38source}, and was labeled
\Code{candidate-knowledge-acquisition}; that label is deliberately weaker than
a causal claim. The experiment trained 58,363,904 rank-8 LoRA parameters with
the base loaded in BF16; the published adapter contains 992 FP32 tensors rather
than BF16 tensors \citep{qwen38adapter,peftmapping,pefttuners}. The
provider-reconciled
whole-Pod cost was
\$3.2853100409265606 (\$3.29 when rounded). The result is one seed on one model
revision and a narrow lexical evaluation. We report the full fifteen-point
checkpoint-evaluation trajectory, public-adapter verification, engineering evidence, and limitations
